class Solution:
    ...
    # ret: cheapest price with at most k or -1 if not possible
        ...
        for i in range(n - 1):
            for edge in flights:
                stepsFrom = dist[edge[0]][1] # 1
                stepsTo = stepsFrom + 1 # 2
                currCost = edge[2] + dist[edge[0]][0] # 1 + 5
                if (currCost < dist[edge[1]][0]) and ((stepsTo == k + 1 and edge[1] == dst) or (stepsTo < k + 1)):
